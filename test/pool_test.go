...
func TestPool_GetWithIPVersion_MismatchProtection(t *testing.T) {
	...
	if err != netconnpool.ErrNoConnectionForIPVersion {
		t.Errorf("应该返回 netconnpool.ErrNoConnectionForIPVersion，但返回: %v", err)
	}
}
...
func TestPool_Close_Safety(t *testing.T) {
	...
	if err != netconnpool.ErrPoolClosed {
		t.Errorf("应该返回 netconnpool.ErrPoolClosed，但返回: %v", err)
	}
}
...
func TestPool_EnforceMaxIdleConnections(t *testing.T) {
	...
	// 创建多个连接并归还
	conns := make([]*netconnpool.Connection, 0, 10)
	for i := 0; i < 10; i++ {
		conn, err := pool.Get(context.Background())
		if err != nil {
			t.Fatalf("获取连接失败: %v", err)
...
func TestPool_ProtocolMismatch(t *testing.T) {
	...
	// 尝试获取UDP连接（应该失败）
	conn, err := pool.GetWithProtocol(ctx, netconnpool.ProtocolUDP, 100*time.Millisecond)
	if err == nil {
		if conn != nil {
			pool.Put(conn)
		}
	...
	if err != netconnpool.ErrNoConnectionForProtocol {
		t.Errorf("应该返回 netconnpool.ErrNoConnectionForProtocol，但返回: %v", err)
	}
}
