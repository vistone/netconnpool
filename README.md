...
1. **从空闲连接池获取**：优先从 `idleConnections` 通道获取已归还的空闲连接
...
3. **归还到空闲池**：将连接放回 `idleConnections` 通道
...
// 协议统计
fmt.Printf("当前TCP连接数: %d\n", stats.CurrentTCPConnections)
fmt.Printf("当前UDP连接数: %d\n", stats.CurrentUDPConnections)
fmt.Printf("当前TCP空闲连接数: %d\n", stats.CurrentTCPIdleConnections)
fmt.Printf("当前UDP空闲连接数: %d\n", stats.CurrentUDPIdleConnections)
...
**必须归还连接**：使用完连接后，必须调用 `Put()` 归还连接，否则会导致连接泄漏。
...
**错误连接的处理**：如果连接在使用过程中出错，应该关闭而不是归还：
...
if err != nil {
    switch err {
    case netconnpool.ErrPoolClosed:
        // 连接池已关闭
...
- **连接存储**：使用 `map[uint64]*Connection` 实现 O(1) 查找和删除
- **空闲连接池**：使用带缓冲通道实现高效的FIFO队列
- **等待队列**：按协议/IP版本分别排队，提高匹配效率

### 2. 并发优化

- **并发健康检查**：使用信号量限制最多10个并发检查
- **并发创建限制**：使用信号量限制最多5个并发创建
- **原子操作**：统计信息使用原子操作更新，避免锁竞争

### 3. 性能优化

- **统计更新频率**：每100ms更新一次时间戳，减少系统调用
- **非阻塞UDP清理**：使用goroutine异步清理，不阻塞连接归还
- **预分配内存**：减少内存分配和GC压力

### 4. 性能对比

典型场景下的性能提升：
- 连接查找/删除：从 O(n) 优化到 O(1)
- 连接复用：40倍以上的性能提升（相比每次创建新连接）
- 健康检查：并发执行，性能提升显著
- 统计更新：减少系统调用，性能提升明显
...
1. **Pool**: 连接池主结构，管理所有连接
   - 连接存储和查找
   - 连接获取和归还
   - 等待队列管理

2. **Connection**: 连接封装，包含连接状态和元数据
   - 连接ID、IP版本、协议类型
   - 复用计数
   - 状态管理（空闲/使用中）

3. **HealthCheckManager**: 健康检查管理器
   - 定期检查连接健康状态
   - 并发执行检查（最多10个并发）
   - 自动移除不健康的连接

4. **CleanupManager**: 清理管理器
   - 清理过期连接（超过MaxLifetime）
   - 清理空闲连接（超过IdleTimeout）
   - 限制空闲连接数（不超过MaxIdleConnections）

5. **LeakDetector**: 泄漏检测器
   - 检测长时间未归还的连接
   - 记录泄漏统计信息

6. **StatsCollector**: 统计信息收集器
   - 收集连接池使用统计
   - 更新统计信息（每100ms更新一次时间戳）
   - 提供详细的统计报告
